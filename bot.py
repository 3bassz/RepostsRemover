...
def main_menu():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📄 استخراج sessionid تلقائيًا", callback_data='auto_session')],
...
def back_menu():
    return InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data='dashboard_main')]])
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "delete_reposts":
        await query.edit_message_text("🗑️ أرسل sessionid الآن لبدء العد والحذف.", reply_markup=back_menu())

    elif data == "auto_session":
        await query.edit_message_text("🧠 أداة استخراج session تلقائيًا (تحت التطوير).", reply_markup=back_menu())

    elif data == "send_ticket":
        context.user_data['ticket_mode'] = True
        await query.edit_message_text("✍️ أرسل استفسارك أو مشكلتك الآن:", reply_markup=back_menu())
        ...
        users = load_json(USERS_FILE)
        await query.edit_message_text(f"👥 عدد المستخدمين: {len(users)}", reply_markup=back_menu())
        ...
        await query.edit_message_text(f"↺ تم تغيير حالة البوت: {status}", reply_markup=back_menu())

    elif data == "edit_welcome":
        if user_id != str(OWNER_CHAT_ID): return
        context.user_data['edit_welcome'] = True
        await query.edit_message_text("✍️ أرسل رسالة الترحيب الجديدة الآن:", reply_markup=back_menu())

    elif data == "view_tickets":
        if user_id != str(OWNER_CHAT_ID): return
        tickets = load_json(TICKETS_FILE)
        if not tickets:
            await query.edit_message_text("📟 لا توجد تذاكر حتى الآن.", reply_markup=back_menu())
            return
        messages = []
        ...
        await query.edit_message_text("✅ تم إرسال التذاكر إلى المالك.", reply_markup=back_menu())

    elif data == "block_user":
        if user_id != str(OWNER_CHAT_ID): return
        context.user_data['block_user'] = True
        await query.edit_message_text("❌ أرسل الآن ID المستخدم الذي تريد حظره:", reply_markup=back_menu())

    elif data == "broadcast":
        if user_id != str(OWNER_CHAT_ID): return
        context.user_data['broadcast'] = True
        await query.edit_message_text("📢 أرسل الآن الرسالة التي تريد إرسالها لكل المستخدمين:", reply_markup=back_menu())
